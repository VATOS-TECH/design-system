...
design_system:
  ...
  document_role: "common-design-rules"
...
default_document_theme:
  name: "Urban Navy"
  usage: "표지와 마무리 페이지는 Urban Navy, 정보량이 많은 본문은 Light Theme를 우선 적용"
  related_resource: "theme-usage-rules"
...
themes:
  shared_principles:
    visual_language:
      ...
      - "브랜드 컬러 역할과 시각적 품질은 유지하되 문서 목적, 대상 독자, 제목 길이, 정보량에 따라 구도와 조형 방식을 유연하게 구성한다"
      - "승인된 예시는 고정 템플릿이 아니라 컬러 균형, 깊이감, 여백, 타이포그래피 위계와 전문성의 품질 기준으로 사용한다"
      - "Urban은 도시 사진이나 스카이라인이 아니라 정돈된 비대칭 구성, 깊은 네이비 명암, 구조적인 기하 표현, 현대적인 여백과 타이포그래피를 의미한다"
    allowed_motifs:
      - "diagonal-layer"
      - "vertical-band"
      - "offset-frame"
    ...
    motif_rules:
      ...
      - "보조 요소는 주요 조형 방식을 강화하는 범위에서만 사용한다"
      - "승인 예시의 제목 위치, 도형 비율, 사선 위치와 면 분할을 그대로 복제하지 않는다"
      - "Cover, Section Divider, Closing은 동일한 시각 언어를 공유하되 동일 구도를 반복하지 않는다"
  ...
  dark_theme:
    ...
    usage:
      ...
      - "Closing"
    rules:
      - "단순한 어두운 직사각형 하나로 화면을 완성하지 않는다"
      - "네이비 명도 차이, 여백, 타이포그래피 또는 절제된 기하 표현을 활용한다"
      - "Cyan은 작은 포인트로 제한한다"
  ...
  light_theme:
    ...
    usage:
      ...
      - "상세 설명"
    rules:
      - "밝은 배경과 명확한 정보 위계를 우선한다"
      - "카드가 필요하지 않은 정보는 직접 배치형 레이아웃으로 구성한다"
      - "본문 전체를 단순 카드 나열형으로 구성하지 않는다"
  ...
  urban_navy_theme:
    ...
    usage:
      ...
      - "브랜드 장표"
    rules:
      - "특정 사선 표지 하나를 의미하지 않는다"
      - "브랜드 컬러와 시각 원칙을 유지하면서 다양한 기하 구성으로 변형한다"
      - "Cover, Section Divider, Closing은 같은 시각 언어를 공유하되 같은 구도를 복제하지 않는다"

  reference:
    detailed_guide: "문서 테마 및 시각 언어의 상세 기준은 theme-usage-rules 역할의 가이드를 따른다"
...
brand_assets:
  ...
  logos:
    primary_signature_slogan:
      path: "assets/logos/vatos-logo-signature-slogan-on-light.png"
      usage: "밝은 배경용 공식 시그니처"
    symbol_wordmark:
      path: "assets/logos/vatos-logo-symbol-wordmark-stacked.png"
      usage: "일반 문서, 본문 상단, 표지 보조 로고"
    wordmark_only:
      path: "assets/logos/vatos-logo-wordmark-only.png"
      usage: "공간이 좁은 영역, 하단 푸터, 보조 브랜드 표기"
    symbol_only:
      path: "assets/logos/vatos-logo-symbol-only.png"
      usage: "아이콘, 워터마크, 썸네일, 작은 브랜드 마크"
    dark_signature_slogan:
      path: "assets/logos/vatos-logo-signature-slogan-on-dark.png"
...
do_not:
  ...
  - "로고를 임의로 변형하지 않는다"
  - "원문에 없는 내용을 임의로 추가하지 않는다"
  - "도회적이라는 표현을 도시 사진, 건축물, 도로, 야경 또는 스카이라인 이미지로 해석하지 않는다"
  - "네이비 직사각형 하나만 배치하여 표지를 완성하지 않는다"
  - "Dark Theme와 Light Theme를 단순한 색상 견본 카드처럼 표현하지 않는다"
  - "표지의 핵심 시각 요소를 일반 카드 컴포넌트로 대체하지 않는다"
  - "승인된 표지 예시의 구도를 모든 문서에 동일하게 복제하지 않는다"
  - "모든 표지에 사선 레이어를 의무적으로 사용하지 않는다"
  - "한 표지에서 여러 종류의 기하 Motif를 과도하게 혼합하지 않는다"
  - "본문 전체를 단순 카드 나열형으로 구성하지 않는다"
---
...
| Repository Area | Resource Role | Description |
|---|---|---|
| `README.md` | `repository-guide` | 디자인 시스템의 목적, 구조, 사용 방법 및 운영 기준 |
| `DESIGN.md` | `common-design-rules` | 전체 산출물에 적용되는 공통 디자인 토큰과 원칙 |
| `SKILL.md` | `ai-execution-rules` | AI가 자료를 탐색하고 디자인 시스템을 적용하는 실행 지침 |
| `assets/` | `brand-assets` | 공식 로고 및 테마 관련 브랜드 자산 |
| `guides/` | `document-type-guides`, `theme-usage-rules` | 문서 유형별 제작 기준과 공통 테마 적용 기준 |
| `templates/` | `document-templates` | 실제 문서 작성에 사용하는 편집 가능한 원본 |
| `references/` | `layout-references`, `component-references` | 레이아웃과 시각적 균형, 승인된 UI 컴포넌트의 형태·상태·상호작용 참고자료 |
| `examples/` | `document-examples` | 디자인 시스템이 적용된 완성 산출물 |

특정 파일의 상세 목록과 실제 경로는 저장소 안내 문서에서 관리한다. 본 문서에서는 가능한 한 자료의 역할을 기준으로 연결한다.

HTML 또는 웹 화면 작성 시 승인된 UI 컴포넌트의 시각 형태, 상태 및 상호작용은 `component-references` 역할의 자료를 확인한다.



## Document Theme Reference

별도 디자인 요청이 없는 경우, 문서의 표지·내지·마무리 페이지는 `theme-usage-rules` 역할의 가이드를 함께 적용한다.
...
테마 관련 이미지는 색감과 분위기를 확인하기 위한 브랜드 자산이며, 실제 구성과 출력 안정성은 문서 유형별 가이드를 우선 적용한다.

승인된 테마 예시는 고정 템플릿이 아니라 컬러 면적 비율, 네이비 깊이감, Cyan 사용량, 여백, 타이포그래피 위계와 전문성을 판단하는 품질 기준이다. 예시의 제목 위치, 도형 비율, 사선 위치 및 면 분할을 그대로 반복 복제하지 않는다.

VATOS에서 Urban은 도시 사진, 건축물, 도로, 야경 또는 스카이라인을 의미하지 않는다. Urban은 정돈된 비대칭 구성, 깊은 네이비 명암, 구조적인 기하 표현, 현대적인 여백과 타이포그래피를 의미한다.
...
Premium Dark는 단독 메인 배경이 아니라, Urban Navy 배경 위에서 기하 레이어, 오버레이, 내부 패널, 면 분할처럼 깊이감과 명암을 만드는 보조 다크 컬러로 사용한다.
...
- 깊이감을 표현할 때는 네이비 면을 딱 끊어 분리하기보다 Urban Navy에서 더 어두운 네이비로 자연스럽게 이어지는 절제된 단색 계열 그라데이션을 우선한다.
- 다색, 고채도, 광택형 그라데이션은 사용하지 않는다.
...
- 문맥형 텍스트 액션은 별도 Text Button으로 정의하지 않고 Navigation 또는 링크 규칙을 적용한다.
...
| 승인된 이미지가 있는 경우 충분한 여백을 확보한다 | 기술, 제품, 스크린샷 등 승인된 이미지는 텍스트와 과도하게 겹치지 않게 배치한다 |
| 승인된 테마 예시는 품질 기준으로 활용한다 | 컬러 균형, 깊이감, 여백, 타이포그래피 위계는 참고하되 구도는 그대로 복제하지 않는다 |
| 표지에는 하나의 주요 조형 방식을 선택한다 | 사선, 세로 밴드, 프레임, 면 분할, 라인 그리드 중 문서 목적에 적합한 방식을 중심으로 구성한다 |
| Cover, Section Divider, Closing은 같은 시각 언어로 연결한다 | 색상과 조형 원칙은 공유하되 동일한 구도를 반복하지 않는다 |
...
| 도회적이라는 표현을 도시 이미지로 해석하지 않는다 | 도시 사진, 건축물, 도로, 야경, 스카이라인은 Urban의 기본 표현이 아니다 |
| 네이비 직사각형 하나만으로 표지를 완성하지 않는다 | 색상만 적용한 단순 박스가 아니라 여백, 타이포그래피, 명암 또는 절제된 기하 구성을 함께 사용한다 |
| Dark Theme와 Light Theme를 색상 견본 카드처럼 표현하지 않는다 | 테마는 실제 문서의 정보 구조와 시각 언어로 적용한다 |
| 승인된 표지 예시의 구도를 반복 복제하지 않는다 | 예시는 품질 기준이며 제목 위치, 사선 위치, 면 분할은 문서 목적에 따라 변형한다 |
| 모든 표지에 사선 레이어를 의무적으로 사용하지 않는다 | 사선은 선택 가능한 조형 방식 중 하나이다 |
| 한 표지에서 여러 종류의 기하 Motif를 과도하게 혼합하지 않는다 | 주요 조형 방식 하나를 선택하고 보조 요소를 제한한다 |
| 본문 전체를 단순 카드 나열형으로 구성하지 않는다 | 정보 성격에 따라 직접 배치, 표, 차트, 다이어그램을 함께 검토한다 |
...
문서 표지, 내지, 마무리 페이지의 실제 적용 기준은 `theme-usage-rules` 역할의 가이드를 우선 적용한다. 별도 디자인 요청이 없는 경우 Urban Navy 표지와 밝은 내지 조합을 기본으로 한다.

### Theme Visual Language

VATOS 테마는 특정 레이아웃을 반복 사용하는 템플릿이 아니라, 브랜드 컬러와 시각적 품질을 유지하면서 문서 목적에 맞게 변형하는 디자인 언어이다.

승인된 테마 예시는 다음 항목을 판단하기 위한 품질 기준으로 사용한다.
...
예시의 사선, 제목 위치, 도형 비율, 하단 구조를 그대로 복제할 필요는 없다.

표지에는 사선 레이어, 세로 밴드, 오프셋 프레임, 네이비 면 분할, 얇은 라인 그리드, 타이포그래피 중심 구성, 화면 밖으로 잘린 추상 기하 구성 등 다양한 조형 방식을 사용할 수 있다. 단, 한 표지에서는 하나의 주요 조형 방식을 선택하고 과도하게 혼합하지 않는다.

Urban은 도시 사진이나 스카이라인을 뜻하지 않는다. 정돈된 비대칭 구성, 깊은 네이비 명암, 구조적인 기하 표현, 현대적인 여백과 타이포그래피를 의미한다.

Cover, Section Divider, Closing은 동일한 시각 언어를 공유하되 같은 구도를 반복하지 않는다. 상세한 Cover, Internal, Section Divider, Closing 및 Variation 기준은 `theme-usage-rules` 역할의 가이드를 따른다.
...
- 단순한 네이비 직사각형 하나로 화면을 완성하지 않는다.
- 네이비 명도 차이, 여백, 타이포그래피 또는 절제된 기하 표현으로 시각적 완성도를 확보한다.
...
- Urban Navy Theme는 특정 사선 표지 하나를 의미하지 않는다.
- 사선 레이어, 세로 밴드, 오프셋 프레임, 면 분할, 라인 그리드, 타이포그래피 중심 구성 등을 문서 목적에 따라 선택할 수 있다.
- 사선 레이어는 선택 사항이며 필수 요소가 아니다.
- 승인 예시의 구도를 그대로 복제하지 않는다.
- Cover, Section Divider, Closing은 같은 시각 언어를 공유하되 동일한 구도를 반복하지 않는다.
- Cover와 Closing에 하단 문서 정보 영역이 있는 경우, 안전 영역 안에 저대비 1px 구분선을 두어 본문과 분리할 수 있다.
- 한 화면에서 여러 종류의 기하 Motif를 과도하게 혼합하지 않는다.
- 기술, 데이터, 제품 또는 스크린샷 등 승인된 이미지가 있는 경우 텍스트와 과도하게 겹치지 않도록 충분한 여백을 확보한다.
...
- 승인된 테마 예시는 고정 템플릿이 아니라 품질 기준으로 사용한다.
- 특정 사선 구성이나 제목 위치를 모든 산출물에 반복 적용하지 않는다.
- 한 표지에는 주요 조형 방식 하나만 선택한다.
- Cover, Section Divider, Closing은 같은 시각 언어를 공유하되 동일 구도를 복제하지 않는다.
- Urban을 도시 사진, 건축물, 야경 또는 스카이라인으로 해석하지 않는다.
...
문서 테마 참고 이미지는 `assets/themes/` 하위에 보관한다. 현재 기본 문서 테마 기준 이미지는 `assets/themes/vatos-theme-default-urban-navy.png`이다.
...
|---|---|---|
| Primary Signature + Slogan | `assets/logos/vatos-logo-signature-slogan-on-light.png` | 밝은 배경용 공식 시그니처. 회사소개서, 제안서, 외부 배포용 문서 표지와 브랜드 영역 |
| Symbol + Wordmark | `assets/logos/vatos-logo-symbol-wordmark-stacked.png` | VA 심볼과 VATOS 워드마크 조합. 일반 문서, 본문 상단, 표지 보조 로고 |
| Wordmark Only | `assets/logos/vatos-logo-wordmark-only.png` | 공간이 좁은 영역, 하단 푸터, 보조 브랜드 표기 |
| Symbol Only | `assets/logos/vatos-logo-symbol-only.png` | 아이콘, 워터마크, 썸네일, 작은 브랜드 마크 |
| Dark Background Signature | `assets/logos/vatos-logo-signature-slogan-on-dark.png` | 어두운 네이비 계열 표지, 섹션 구분, Closing 화면 |

Markdown 문서에서 로고를 표시해야 할 경우에는 이미지를 문서에 직접 삽입하지 않고, 상대 경로로 참조한다.

```markdown
![VATOS Logo](assets/logos/vatos-logo-signature-slogan-on-light.png)
```
...
|---|---|---|
| Urban Navy Document Theme | `assets/themes/vatos-theme-default-urban-navy.png` | 별도 디자인 요청이 없는 경우 표지, 내지, 마무리 페이지의 기본 톤앤매너 참고 이미지 |

Theme Asset은 실제 문서 작성 시 적용할 시각 기준을 보여주는 참고 이미지이다. 세부 적용 규칙은 `theme-usage-rules` 역할의 가이드를 따른다.
...
|---|---|
| Brand Image | 승인된 회사, 기술, 데이터, 제품 관련 이미지를 사용한다 |
...
여기서 도회적이라는 표현은 도시 사진, 건축물, 도로, 야경 또는 스카이라인을 뜻하지 않는다. 정돈된 비대칭 구성, 깊은 네이비 명암, 구조적인 기하 표현, 현대적인 여백과 타이포그래피를 의미한다.
...
- Theme 상세 기준은 `## Theme Variants`와 `theme-usage-rules` 역할의 가이드를 따른다.
- 승인된 예시는 고정 템플릿이 아니라 시각적 품질 기준으로 사용한다.
- 표지 제작 시 특정 사선 구성이나 제목 위치를 자동 반복하지 않는다.
- Cover, Section Divider, Closing은 같은 시각 언어를 공유하되 동일 구도를 복제하지 않는다.
...
별도 디자인 요청이 없는 경우 Urban Navy 표지와 밝은 내지 조합을 기본으로 하며, 해당 문서 유형의 제작 가이드와 관련 자료를 함께 확인한다.

AI는 원문에 없는 내용을 임의로 추가하지 않는다.  
디자인 판단이 필요한 경우에는 임의로 확정하지 않고, 사용자가 검토할 수 있는 시각화 예시 또는 대안을 먼저 제시한다.
...
|---|---|---|---|---|
| alpha | 2026.07.16 | 2026.07.16 | Changed | HTML 컴포넌트 기준과 일치하도록 Text Button 유형을 제거하고, 테마 가이드 참조를 역할명 중심으로 정리함. |
| alpha | 2026.07.14 | 2026.07.14 | Changed | 현재 저장소 구조에 맞춰 `assets/themes/`, `guides/theme.md`, `examples/`, `references/`, `SKILL.md` 참조를 반영하고 AI Output Priority와 기본 문서 테마 기준을 업데이트함. |
